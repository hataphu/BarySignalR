...
namespace BarySignalR.SignalR
{
    ...
    {
        /// <summary>
        /// Configures SignalR to use the BarySignalR backplane.  Requires that an <see cref="IClusterClient"/> is registered.
        ...
        /// <returns>The same same builder, configured to use BarySignalR</returns>
        public static ISignalRBuilder UseBarySignalR(this ISignalRBuilder builder)
        {
            ...
            builder.Services.AddSingleton(
                ...
                typeof(BarySignalRHubLifetimeManagerFactory<>)
            );
        ...
        // Below are generic versions of the non generic BarySignalR types.
        // We use these to allow SignalR to get the correct BarySignalR services for the specific hub that requires them.
        ...
        public class BarySignalRHubLifetimeManagerFactory<T> : HubLifetimeManager<T> where T : Hub
        {
            readonly Task<BarySignalRHubLifetimeManager<T>> @delegate;

            public BarySignalRHubLifetimeManagerFactory(IServiceProvider services)
            {
                @delegate = BarySignalRHubLifetimeManager<T>.CreateAsync(
                    ...
                    services.GetRequiredService<ILogger<BarySignalRHubLifetimeManager<T>>>()
                );
            }
